/*
maxhold~ - Maximum hold (peak hold) external for Pure Data
2024
- Outputs max(previous, current input) * coefficient
...
static t_class *maxhold_tilde_class;

typedef struct _maxhold_tilde {
    t_object x_obj;
    t_sample f_dummy;
    t_float *state;
    t_sample **in;
...
static t_int *maxhold_tilde_perform(t_int *w)
{
    t_maxhold_tilde *x = (t_maxhold_tilde *)(w[1]);
    int n = (int)(w[2]);
    int i, j;
...
static void maxhold_tilde_dsp(t_maxhold_tilde *x, t_signal **sp)
{
    int i;
    ...
    dsp_add(maxhold_tilde_perform, 2, x, vec_size);
}

static void maxhold_tilde_reset(t_maxhold_tilde *x, t_floatarg f)
{
    for (int i = 0; i < x->n_channels; i++) x->state[i] = f;
}

static void *maxhold_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    t_maxhold_tilde *x = (t_maxhold_tilde *)pd_new(maxhold_tilde_class);
    t_float coef = 1;
...
static void maxhold_tilde_free(t_maxhold_tilde *x)
{
...
void maxhold_tilde_setup(void)
{
    maxhold_tilde_class = class_new(gensym("maxhold~"),
        (t_newmethod)maxhold_tilde_new,
        (t_method)maxhold_tilde_free,
        sizeof(t_maxhold_tilde),
        CLASS_MULTICHANNEL,
        A_GIMME, 0);
    class_addmethod(maxhold_tilde_class, (t_method)maxhold_tilde_dsp, gensym("dsp"), A_CANT, 0);
    class_addmethod(maxhold_tilde_class, (t_method)maxhold_tilde_reset, gensym("reset"), A_FLOAT, 0);
    CLASS_MAINSIGNALIN(maxhold_tilde_class, t_maxhold_tilde, f_dummy);
} 